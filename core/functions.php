...
add_action( 'admin_menu', 'linked_url' );
    function linked_url() {
    add_menu_page( 'linked_url', 'تحميل الطلبات ', 'read', 'my_slug', '', 'dashicons-text', 1 );
    }

    add_action( 'admin_menu' , 'linkedurl_function' );
    function linkedurl_function() {
    global $menu;
    $menu[1][2] = "https://eskelah.com/?action=download_csv";
    }
...
function get_simple_contact() {
    require_once get_template_directory() .'/core/template/simple_contact.php';
}
...
if(self::videoType($url) == 'youtube') {
    ...
    echo 'youtube video here' ;
}
    
if(self::videoType($url) == 'vimeo') {
    if(preg_match("/(https?:\/\/)?(www\.)?(player\.)?vimeo\.com\/([a-z]*\/)*([0-9]{6,11})[?]?.*/", $url, $output_array)) {
    echo "<div class='embed-container'><iframe src='https://player.vimeo.com/video/".$output_array[5]."' frameborder='0' webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>";
